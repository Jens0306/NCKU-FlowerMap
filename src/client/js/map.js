...
var flower1, flower2, flower3, flower4, flower5, flower6;
$(document).ready(function() {
  $.ajax({
    ...
    success : function(data) {
      ...
      flower1 = new L.geoJson(location1, {
...
function onMapClick(e) {
  ...
  popup
    ...
    .setContent("<form id='add' action='addFlower' method='post'><input type='hidden' name='lat' value="
    	...
    	+lng
    	+">"
    	+"<input type='radio' name='flower' value='f1'/>f1<br>"
...
$(document).ready(function() {
  $('form').ajaxForm({
      delegation: true,
      target:        '.savePics',   // target element(s) to be updated with server response
...
function showResponse(responseText, statusText, xhr, $form) {
  //dynamically reload map when new flower is added
  switch(responseText[1]) {
    case 1:
    map.removeLayer(flower1);
    flower1 = new L.geoJson(responseText[0], {
      onEachFeature: basement1
    })
    map.addLayer(flower1);
    //console.log('flower'+responseText[1]);
    break;
    case 2:
    map.removeLayer(flower2);
    flower2 = new L.geoJson(responseText[0], {
      onEachFeature: basement2
    })
    map.addLayer(flower2);
    //console.log('flower'+responseText[1]);
    ...
    case 3:
    map.removeLayer(flower3);
    flower3 = new L.geoJson(responseText[0], {
      onEachFeature: basement3
    })
    map.addLayer(flower3);
    break;
    case 4:
    map.removeLayer(flower4);
    flower4 = new L.geoJson(responseText[0], {
      onEachFeature: basement4
    })
    map.addLayer(flower4);
    break;
    case 5:
    map.removeLayer(flower5);
    flower5 = new L.geoJson(responseText[0], {
      onEachFeature: basement5
    })
    map.addLayer(flower5);
    break;
    case 6:
    map.removeLayer(flower6);
    flower6 = new L.geoJson(responseText[0], {
      onEachFeature: basement6
    })
    map.addLayer(flower6);
    break;
  }
  map.closePopup();              //close the popup form after submition
}
//*/
